...
(X_train, y_train, g_train, X_test, y_test, g_test, _) = load_dataset(
    ...
    removeControl=False, scale='n', multiclass=False
)

# mdl = VotingRegressor([('kn', KNeighborsRegressor()), ('svr', SVR())])
mdl = StackingRegressor([('kn', KNeighborsRegressor()), ('svr', SVR())], final_estimator=BaggingRegressor())

mdl.fit(X_train, y_train)
y_pred_f = mdl.predict(X_test)
y_pred = y_pred_f.round()
