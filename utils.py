...
def str2attrs(attrs: str):
    ...
    if attrs == 't':
        return ['x_D', 'y_D', 'z_D', 'x_ND', 'y_ND', 'z_ND']
...
# from util_classes import get_bins
...
FEATURES_FUNCTIONS = lambda x: np.append(
    np.append(
        np.array([
            ...
            # np.min(np.diff(x)),
            np.count_nonzero(x) / np.size(x)
    # Decili
        ]), 
    np.quantile(x, np.arange(0, 1.1, .1))),
(np.quantile(np.diff(x), np.arange(0, 1.1, .1)) + 1) / 2)
...
# # Decili
#     ]), 
# np.quantile(x, np.arange(0, 1.2, .2)))
...
#     # Decili
#         ]), 
#     get_bins(x, n=10, r=(0,1))),
# get_bins(np.diff(x), n=10, r=(-1,1)))
...
def get_subject_features(metadata: pd.DataFrame, subject: int, attrs_sbj: str):
    features = []
    MAX_AGE = 27
    MIN_AGE = 5
    attrs = ['gender', 'dom']
    if attrs_sbj == 'ext':
        attrs = ['gender', 'dom', 'side_hemi', 'hemi']
...
def single_subject(s_data: pd.DataFrame, metadata: pd.DataFrame, length: int, class_method: str, attrs_str: str, max_zeros: int, attrs_sbj: str, overlap: int) -> Tuple[int, np.array, np.array]:
    s_dataset = list()
    s_group = list()
    s_start_idx = s_data.index[0]
    subject = s_data.subject.iat[0]
    s_class = get_class(subject, metadata, class_method)
    s_features = get_subject_features(metadata, subject, attrs_sbj=attrs_sbj)
    start = 0
    i = 0
    while True:
        i+=1
    ...
    # cprint(f'Subject {subject} done!', 'green')
    return (subject, s_dataset, s_group)

def count_windows_per_subject(s_data: pd.DataFrame, metadata: pd.DataFrame, length: int, class_method: str, attrs_str: str, max_zeros: int, attrs_sbj: str):
    subject = s_data.subject.iat[0]
    s_class = get_class(subject, metadata, class_method)
    s_start_idx = s_data.index[0]
    start = 0
...
def par_make_dataset(data: pd.DataFrame, metadata: pd.DataFrame, length: int, overlap: int, class_method: str, attrs: str, max_zeros: int, attrs_sbj: str) -> Tuple[np.array, np.array, np.array]:
    argList = []
    if overlap < 0:
        base_overlap = np.abs(overlap)
        for _, m_row in metadata.iterrows():
            s_data = data[data.subject == m_row.subject]
            a = [s_data, metadata, length, class_method, attrs, max_zeros, attrs_sbj, {}]
            argList.append(a)
        ...
        overlaps = np.round(class_counts / np.max(class_counts) * base_overlap)
        print(f'Overlaps: {overlaps}')
        for i,al in enumerate(argList):
            al[7] = {'overlap': max(1,overlaps[res[i][0]])}
    else:
        for _, m_row in metadata.iterrows():
            s_data = data[data.subject == m_row.subject]
            a = [s_data, metadata, length, class_method, attrs, max_zeros, attrs_sbj, {'overlap': overlap}]
            argList.append(a)
...
def load_dataset(
        ...
        class_method = 'macs', attrs='DND', attrs_sbj='std',
        split=True, reload=False, 
        removeControl=False, multiclass=False, 
        scale='', version = None, test_pcentage=.3, max_zeros=.3, random_state=None
    ) -> Tuple[np.array, np.array, np.array, np.array, BaseEstimator]:
    skip_table = isinstance(metadata, pd.DataFrame)
    if version == None: version = VERSION
    filename = f'./datasets/{str(data)}_{length}_{overlap}_{class_method}_{attrs}_{attrs_sbj}_z{max_zeros}_v{version}.npz'
    ...
    if path.isfile(filename) and not reload and not skip_table:
        ...
        (X, y, g) = par_make_dataset(data.table(), mdata, length, overlap, class_method, attrs, max_zeros, attrs_sbj)
        if version == VERSION:
    ...
    if split == False:
        if removeControl:
            ...
            y = y[idx_nctrl]
            g = g[idx_nctrl]
        return X, y, g, np.array([]), np.array([]), np.array([]), filename
    gss = StratifiedGroupKFold(n_splits=round(1/test_pcentage), shuffle=True, random_state=random_state)
    ...
    return X_train, y_train, g_train, X_test, y_test, g_test, filename
