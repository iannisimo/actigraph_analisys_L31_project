...
(X_train, y_train, g_train, X_test, y_test, g_test, _) = load_dataset(
    tables.AHA, tables.METADATA, length=180, overlap=60, 
    class_method='macs', attrs='ND', scale='',
    test_pcentage=.3333, max_zeros=.3
)
...
space = [
    ...
    Categorical(['squared_error', 'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'], name='loss')
]
